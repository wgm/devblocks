<?php
include_once(CG_PATH . "/libs/pear/i18n/I18N_UnicodeString.php");
include_once(CG_PATH . "/languages/".UM_LANGUAGE."/strings.php");
...
//							print_r($manifest);
	...
	static private function _readPluginManifest($dir) {
		...
		include_once(CG_PATH . '/libs/domit/xml_domit_include.php');
		...
		foreach($manifest->extensions as $pos => $extension) { /* @var $extension CgExtensionManifest */
			...
			$rs = $um_db->Execute($sql) or die(__CLASS__ . ':' . $um_db->ErrorMsg()); /* @var $rs ADORecordSet */
			if($rs->NumRows()) { // exists
				$manifest->extensions[$pos]->extension_id = stripslashes($rs->fields['id']);
				$sql = sprintf("UPDATE extension SET plugin_id=%d,point=%s,pos=%d,name=%s,file=%s,class=%s,params=%s WHERE id=%s",
					$extension->plugin_id,
					$um_db->QMagic($extension->point),
					$pos,
					...
					$um_db->QMagic($manifest->extensions[$pos]->extension_id)
				);
				$um_db->Execute($sql) or die(__CLASS__ . ':' . $um_db->ErrorMsg());
			} else { // new
				$sql = sprintf("INSERT INTO extension (id,plugin_id,point,pos,name,file,class,params) ".
...
class _CgSessionManager {
	...
	static function getInstance() {
		...
		if(null == $instance) {
			$um_db = CgPlatform::getDatabaseService();
			if(!$um_db->IsConnected()) return null;
			
			include_once(CG_PATH . "/libs/adodb/session/adodb-session2.php");
			$options = array();
...
class _CgEmailManager {
	...
	static function send($server, $sRCPT, $headers, $body) {
		// mailer setup
		require_once(CG_PATH . '/libs/pear/Mail.php');
		$mail_params = array();
	...
	static function getMessages($server, $port, $service, $username, $password) {
		...
		require_once(CG_PATH . '/libs/pear/mimeDecode.php');
...
class _CgTemplateManager {
	...
	static function getInstance() {
		static $instance = null;
		if(null == $instance) {
			require(CG_PATH . '/libs/smarty/Smarty.class.php');
			$instance = new Smarty();
			$instance->template_dir = CG_PATH . '/templates';
			$instance->compile_dir = CG_PATH . '/templates_c';
			$instance->cache_dir = CG_PATH . '/cache';
			
			//$smarty->config_dir = CG_PATH . '/configs';
			$instance->caching = 0;
...
class _CgDatabaseManager {
	...
	static function getInstance() {
		static $instance = null;
		if(null == $instance) {
			include_once(CG_PATH . "/libs/adodb/adodb.inc.php");
			$ADODB_CACHE_DIR = CG_PATH . "/cache";
			@$instance =& ADONewConnection(UM_DB_DRIVER); /* @var $instance ADOConnection */
